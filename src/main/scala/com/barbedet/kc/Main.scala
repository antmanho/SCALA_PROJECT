...
import com.barbedet.kc.http.{WebSocketRoutes, StaticRoutes, CorsMiddleware, TusmoRoutes}
import com.barbedet.kc.model.{GameState, PlayerStats}
  ...
  override def run(args: List[String]): IO[ExitCode] =
    for {
      ...
      _ <- EmberServerBuilder
        ...
        .withPort(port"8080")
        .withHttpWebSocketApp(httpApp(gamesRef, statsRef))
        .build
        .use { _ =>
